...
import { formatCurrency, formatNumberToSocialType, getIdFormNameId, rateSale } from 'src/utils/utils'
...
export default function ProductDetail() {
  ...
  const handleZoom = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    ...
    const imgage = imgRef.current as HTMLImageElement
    const { naturalWidth, naturalHeight } = imgage
    const { offsetX, offsetY } = event.nativeEvent
    const top = offsetY * (1 - naturalHeight / rect.height)
    const left = offsetX * (1 - naturalWidth / rect.width)
    imgage.style.width = naturalWidth + 'px'
    imgage.style.height = naturalHeight + 'px'
    imgage.style.maxWidth = 'unset'
    imgage.style.top = top + 'px'
    imgage.style.left = left + 'px'
  }

  const handleRemoveZome = () => {
        ...
        <div className='bg-white p-4 shadow'>
          <div className='grid grid-cols-12 gap-9'>
            <div className='col-span-5'>
              {/* Css để ảnh có chìu cao = chìu rộng */}
              <div
                className='relative w-full cursor-zoom-in overflow-hidden pt-[100%] shadow'
                onMouseMove={handleZoom}
                onMouseLeave={handleRemoveZome}
              >
                <img
                  src={activeImage}
                  alt={product.name}
              ...
              <div className='mt-8 flex items-center'>
                <div className='flex items-center'>
                  ...
                  <ProductRating
                    ...
                    noneActiveClassname='fill-gray-300 h-4 w-4 text-gray-300'
                  />
                </div>
